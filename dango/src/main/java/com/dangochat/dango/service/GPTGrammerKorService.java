...
public class GPTGrammerKorService {
        ...
        switch (promptType) {
            case 1:
                prompt = content.getContent() + "는 " + content.getMeaning() +
                        ...
                        "문제는 반드시 한국어 문법 문제로 작성해 주세요. 일반 상식 문제는 포함하지 마세요.";
                break;
            ...
            if (content != null && !content.getContent().trim().isEmpty()) {
                ...
                try {
                    ...
                    if (gptResponse != null && gptResponse.getChoices() != null) {
                        ...
                        log.error("문제가 생성되지 않았습니다.");
                    }
                } catch (Exception e) {
                    log.error("GPT API 통신 중 오류 발생: " + e.getMessage());
    ...
    // 문법 문제를 생성하고 반환
    public List<String> loadQuestions(HttpSession session, int startIndex, int count) {
